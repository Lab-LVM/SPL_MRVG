...
def generate_latents_stylegan3(n_latents, network_pkl, latent_dim=512, truncation_psi=1.0):
    """Generates random, mapped latents using StyleGAN3
    ...
    Args:
        ...
        truncation_psi (float, optional): Truncation psi. Defaults to 1.0.
    ...
    zs = th.randn((n_latents, latent_dim), device=device)

    # Map z vectors to w space using the generator's mapping network
    ws = G.mapping(zs, None, truncation_psi=truncation_psi)
...
def chroma_weight_latents(chroma, latents): # chroma shape (12,216) latents shape (18,512)
...
def slerp(val, low, high):
    ...
    from https://github.com/soumith/dcgan.th/issues/14#issuecomment-200025792
...
def generate_latents(n_latents, ckpt, G_res, noconst=False, latent_dim=512, n_mlp=8, channel_multiplier=2):
    ...
    zs = th.randn((n_latents, latent_dim), device="cuda") # n_latents =12 12개의 잠재백터 18,512를 만들어내는, 나중에 이것이 12개의 chromaweight들이랑 곱해진다.
