...
def plot_spectra(spectra, path, chroma=False):
    ...
    for ax, spectrum in zip(axes if len(spectra) > 1 else [axes], spectra):
        ...
        if spectrum.shape[1] == 12:
            spectrum = spectrum.T
        img=rosa.display.specshow(spectrum, y_axis="chroma" if chroma else None, x_axis="time", ax=ax,cmap='plasma') # 색상조절 : cmap='coolwarm'

        ax.set_y("Chroma" if chroma else "Frequency", fontsize=15)  
        ax.set_xlabel("Time", fontsize=15)  
        fig.colorbar(img, ax=ax)
    plt.tight_layout()
    plt.gca().tick_params(axis='both', direction='out', length=5, pad=5)
    plt.gca().yaxis.set_ticks_position("left")
    plt.gca().xaxis.set_ticks_position("bottom")
    plt.savefig(path.replace(".png", ".svg"), format="svg")  # Save as SVG
    plt.show()
